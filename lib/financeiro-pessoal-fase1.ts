...
import { isLancamentoPessoalAtrasado, isDespesaPessoalVencida } from "@/lib/financeiro-pessoal-utils";
...
/**
 * Mesma forma de dado que lib/financeiro-fase1.ts (FinanceiroCards/
 * FinanceiroAlertas), de propósito — permite reaproveitar os componentes de
 * UI do financeiro do escritório (FinanceiroSummaryCards, FinanceiroAlerts,
 * RevenueExpenseRatio) sem duplicá-los, mesmo com tabelas totalmente
 * separadas por trás.
 */
export interface FinanceiroPessoalCards {
  receitaMes: number;
...
export function computeFinanceiroPessoalCards({
  lancamentos,
  despesas,
}: {
  lancamentos: LancamentoPessoal[];
  despesas: DespesaPessoal[];
}): FinanceiroPessoalCards {
  ...
  const inadimplentes = lancamentos.filter(isLancamentoPessoalAtrasado);
...
export interface FinanceiroPessoalAlertas {
...
export function computeFinanceiroPessoalAlertas({
  lancamentos,
  despesas,
}: {
  lancamentos: LancamentoPessoal[];
  despesas: DespesaPessoal[];
}): FinanceiroPessoalAlertas {
  const now = Date.now();
  ...
  return {
    faturasVencidas: lancamentos.filter(isLancamentoPessoalAtrasado).length,
    despesasVencidas: despesas.filter(isDespesaPessoalVencida).length,
    receitasProx7Dias,
    despesasProx7Dias,
  };
}
