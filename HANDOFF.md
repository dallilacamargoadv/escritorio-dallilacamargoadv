...
> Documento de continuidade de sessão. Cole este arquivo (ou peça para o Claude ler `HANDOFF.md` na raiz do projeto) no início de uma nova conversa para retomar exatamente de onde paramos, sem precisar reprocessar todo o histórico anterior. **Atualizado em 2026-07-20**, depois de uma sessão que começou a Onda 6 do roadmap de CRM jurídico (blog rich-text/SEO/Configurações) pela primeira das 3 partes: editor rich-text do post, já em produção. As partes de SEO por página e Hub de Configurações da Onda 6 ainda não foram feitas. **Se você está começando uma conversa nova a partir deste arquivo: leia a seção 11 primeiro, ela diz exatamente por onde retomar.**
...
- **`posts`**: blog. Sem mudança de schema na onda 6 — o editor rich-text (Tiptap) escreve/lê Markdown no mesmo campo `content` de sempre, `MDXRemote` no site público não mudou. Ainda pendentes da onda 6: SEO por página (meta title/description) e Hub de Configurações — ver seção 8.5.
...
**Tudo commitado, pushado e em produção (Vercel, `READY`)** até o commit `4bda1ba` — **nada pendente de commit ao final desta sessão**. Domínio próprio `dallilacamargoadv.com.br` confirmado ativo e servindo o deployment mais recente (cada commit abaixo foi verificado individualmente com `list_deployments`/`get_deployment` até `READY` antes de seguir pro próximo). Em ordem cronológica:
...
6. 🔶 **Blog rich-text + SEO + Configurações** — 3 partes. **Parte 1/3 (editor rich-text) concluída**, ver seção abaixo. Partes 2/3 (SEO por página) e 3/3 (hub de Configurações) ainda pendentes.
...
23. **Onda 6, parte 1/3: editor rich-text do blog** (Tiptap, upload de imagem pro bucket público novo `blog-imagens`, mantendo Markdown por baixo) — **concluída** (seção 8.5). Cliente escolheu essa parte entre as 3 da onda 6. **Sessão encerrada neste ponto** — próximo passo é continuar a onda 6 (SEO por página ou Hub de Configurações) ou outro pedido novo, ver seção 11.
...
1. Ler este arquivo primeiro (auto-suficiente) — a seção 7 já confirma que está tudo commitado/pushado/em produção até `4bda1ba`, não precisa checar de novo a menos que desconfie de algo.
2. **Próximo passo**: a sessão terminou logo depois da cliente confirmar a parte 1/3 da Onda 6 (editor rich-text do blog) funcionando em produção ("Sim, tudo certinho"), **sem ela ter dito se quer continuar a onda 6 agora**. Comece a próxima conversa **perguntando o que ela quer fazer agora** — não presumir. Candidatos naturais: continuar a Onda 6 (parte 2/3 SEO por página, ou parte 3/3 Hub de Configurações — ver seção 8.5 pro que já foi decidido/feito), Onda 7 (Financeiro pessoal, tabelas separadas), Onda 8 (integração automática com o Comunica PJe — ver decisão de fonte na seção 8.5, já testada e com endpoint confirmado). Mas ela também pode trazer algo totalmente novo, ou um ajuste fino em algo já entregue — trate com a mesma seriedade (ver seção 10, ela é direta sobre pedir ajustes pós-entrega).
