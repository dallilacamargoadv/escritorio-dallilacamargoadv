...
import { deleteDespesa, getDespesaById, updateDespesa, type DespesaInput } from "@/lib/db-despesas";
...
export async function GET(
  ...
  ctx: RouteContext<"/api/admin/despesas/[id]">,
) {
...
export async function PATCH(
  ...
  try {
    const despesa = await updateDespesa(id, input);
    return NextResponse.json({ despesa });
  } catch (error) {
    console.error(error);
    return NextResponse.json(
