...
const SERVICE_TYPE_OPTIONS = [
  {
    value: "elaboracao",
    label: "Elaboração de um novo contrato",
    hint: "Preciso criar um modelo do zero, personalizado para o meu negócio.",
  },
  {
    value: "revisao",
    label: "Revisão de um contrato existente",
    hint: "Recebi um contrato de terceiros ou tenho um modelo antigo e quero analisar os riscos/cláusulas antes de assinar.",
  },
  {
    value: "parceria",
    label: "Análise de Parceria / Sociedade",
    hint: "Vou iniciar um projeto com outra pessoa/empresa e preciso formalizar os termos.",
  },
];
...
const ACTIVITY_OPTIONS = [
  {
    ...
    hint: "Social Media, Designer, Gestor de Tráfego, Videomaker, Programador, etc.",
  },
  { value: "criador", label: "Criador de Conteúdo / Influenciador Digital" },
  {
    value: "infoprodutor",
    label: "Infoprodutor / Coprodutor / Agência de Lançamento",
  },
  { value: "empresa", label: "Empresa / E-commerce / Startup" },
...
const URGENCY_OPTIONS = [
  {
    value: "urgente",
    label: "Sim, é urgente",
    hint: "Preciso fechar o negócio nos próximos dias.",
  },
  {
    value: "confortavel",
    label: "Tenho um prazo confortável",
    hint: "Próximas semanas.",
  },
  { value: "sem_prazo", label: "Não tenho prazo fixo" },
];
...
export function ContratosForm() {
  ...
  const handleSubmit = async (finalFormData: FormData) => {
    ...
    setError("");
    try {
      const response = await fetch("/api/contratos-leads", {
        method: "POST",
  ...
  return (
    <div>
      ...
      {step === 4 && (
        <StepFrame step={step} error={error} onBack={handlePrev}>
          <p className="text-lg text-ink">De qual serviço você precisa no momento?</p>
      ...
      {step === 5 && (
        <StepFrame step={step} error={error} onBack={handlePrev}>
          <p className="text-lg text-ink">
            Qual opção melhor descreve a sua atividade principal?
          </p>
          <div className="mt-4 space-y-3">
            {ACTIVITY_OPTIONS.map((option) => (
              <OptionButton
                ...
                onClick={() => handleOptionSelect("activity", option.value)}
              />
            ))}
          </div>
        </StepFrame>
      ...
      {step === 6 && (
        <StepFrame step={step} error={error} onBack={handlePrev}>
          <p className="text-lg text-ink">
            Você já tem uma data limite ou prazo em mente para a assinatura
            deste contrato?
          </p>
          <div className="mt-4 space-y-3">
            {URGENCY_OPTIONS.map((option) => (
              <OptionButton
                ...
                onClick={() => {
                  const next = { ...formData, urgency: option.value };
                  setFormData(next);
                  setError("");
                  handleSubmit(next);
                }}
