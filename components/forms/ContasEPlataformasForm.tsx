...
const INCIDENT_OPTIONS = [
  {
    value: "conta_comprometida",
    label: "Minha conta foi comprometida",
    hint: "Perdi o acesso, mudaram e-mail/senha ou não consigo mais entrar.",
  },
  {
    value: "bloqueio",
    label: "Minha conta foi bloqueada / suspensa",
    hint: "A própria plataforma desativou ou restringiu meu perfil.",
  },
  {
    value: "strike",
    label: "Recebi um strike ou restrição de conteúdo",
    hint: "Remoção de publicações, alcance limitado ou aviso de violação de termos.",
  },
...
export function ContasEPlataformasForm() {
  ...
  const handleSubmit = async (finalFormData: FormData) => {
    ...
    setError("");
    try {
      const response = await fetch("/api/contas-e-plataformas-leads", {
        method: "POST",
      ...
      if (data?._eventId) {
        trackLead({
          ...
          contentName: "Contas e Plataformas",
          value: data._value ?? 0,
        });
      }
  ...
  if (step === TOTAL_STEPS) {
    return <FormSuccess areaLabel="Contas e Plataformas" />;
  }
