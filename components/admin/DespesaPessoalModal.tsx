...
import type {
  DespesaPessoal,
  DespesaPessoalRecorrencia,
  DespesaPessoalStatus,
} from "@/lib/db-despesas-pessoal";
import type { DespesaCategoria } from "@/lib/db-despesa-categorias";
import { FORMAS_PAGAMENTO } from "@/lib/despesas-categorias";
...
const STATUS_KEYS: DespesaPessoalStatus[] = ["a_pagar", "pago", "cancelado"];
const RECORRENCIA_KEYS: DespesaPessoalRecorrencia[] = [
  "nenhuma",
  "mensal",
  "trimestral",
  "semestral",
...
export function DespesaPessoalModal({
  despesa,
  categorias,
  despesasHistorico,
  onClose,
  ...
  despesasHistorico: DespesaPessoal[];
  onClose: () => void;
  onSaved: (despesa: DespesaPessoal) => void;
}) {
  ...
  const [status, setStatus] = useState<DespesaPessoalStatus>(despesa?.status ?? "a_pagar");
  const [formaPagamento, setFormaPagamento] = useState(despesa?.forma_pagamento ?? "");
  const [recorrencia, setRecorrencia] = useState<DespesaPessoalRecorrencia>(
  ...
  async function handleSave() {
    ...
    const url = despesa
      ? `/api/admin/despesas-pessoal/${despesa.id}`
      : "/api/admin/despesas-pessoal";
  ...
  return (
    ...
    >
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-label={despesa ? "Editar despesa pessoal" : "Nova despesa pessoal"}
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center justify-between border-b border-hairline px-5 py-4">
          <p className="text-[15px] italic text-ink">
            {despesa ? "Editar despesa" : "Nova despesa pessoal"}
          </p>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4">
          ...
          <div className="mt-3">
            ...
            <input
              ...
              placeholder="Ex.: aluguel, supermercado, streaming"
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <select
                value={status}
                onChange={(e) => setStatus(e.target.value as DespesaPessoalStatus)}
                className="mt-1.5 w-full border border-hairline-strong bg-bg px-3 py-2 text-sm text-ink"
              >
                {STATUS_KEYS.map((key) => (
          ...
          <div className="mt-3 grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <select
                value={recorrencia}
                onChange={(e) => setRecorrencia(e.target.value as DespesaPessoalRecorrencia)}
                className="mt-1.5 w-full border border-hairline-strong bg-bg px-3 py-2 text-sm text-ink"
              >
          ...
          <input
            ...
            className="w-full border border-hairline-strong bg-bg px-3 py-2 text-sm text-ink outline-none transition-colors duration-150 focus:border-gold"
          />
